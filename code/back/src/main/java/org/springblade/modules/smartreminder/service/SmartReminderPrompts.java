...
public final class SmartReminderPrompts {
	...
	public static final String INTENT = """
		...
		9. 创建或修改事件前，必须检查同一接收人的其他活动事件以及本次输入中准备同时创建的多个事件。只有不同事项有明确的同一开始时间或明确占用区间重叠时，才用clarify说明冲突并询问是否保留两项。不得猜测结束时间或持续时长；15点开会与16点聊天，未提供会议结束时间时不能认定冲突。不同日期、不同接收人、仅截止时间接近或活动状态尚未结束不构成冲突，提醒/评估时间不代表日程占用。结构化日期优先，不把历史“今天/明天”套用到当前日期。用户明确说继续、都保留或都发送后，再承接上下文执行，不得反复追问。
	...
	public static final String DECISION = """
		...
		只有“同一接收人的其他活动事件”与当前不同事项有明确的同一开始时间或明确占用区间重叠，且时间轴没有用户要求两项都继续时，timeConflict才为true、action为ASK_RECIPIENT。不得猜测未提供的结束时间或时长；15点开会与16点聊天不能仅凭开始时间认定冲突。不同日期、截止时间、评估/提醒时间或尚未结束的状态均不能单独证明占用；以结构化日期为准，不重解历史相对日期。没有明确重叠证据则timeConflict=false。冲突信息只允许发给受影响的接收人和事件发起人，严禁出现在同事件其他接收人的消息中。若lastConflictTime显示近期已经询问且尚未回复，应DEFER等待；若时间轴记录用户明确选择继续，可正常判断并避免重复询问。
		""";
}
