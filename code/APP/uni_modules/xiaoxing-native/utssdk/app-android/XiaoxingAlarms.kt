...
object XiaoxingAlarms {
    ...
    @Synchronized fun handle(activity: Activity, action: String, input: String): String {
        ...
        if (action == "alarm.list") {
            ...
            save(activity, data)
            return JSONObject().put("supported", true).put("alarms", items).toString()
        }
...
class XiaoxingAlarmReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        } catch (_: Exception) { /* No private event data in system logs. */ }
    }
}
class XiaoxingAlarmService : Service() {
    companion object { @Volatile var currentId = "" }
    ...
    override fun onDestroy() {
        ...
        wakeLock?.let { if (it.isHeld) it.release() }; currentId = ""; stopForeground(true); super.onDestroy()
    }
}
