        ...
        try {
            ...
            if(action=="update.install") {
                ...
                Thread {
                    ...
                    val target=File(activity.cacheDir,"xiaoxing-update.apk")
                    try {
                        ...
                        var total=0L
                        connection!!.inputStream.use { source -> target.outputStream().use { sink ->
                        ...
                        val candidate=activity.packageManager.getPackageArchiveInfo(target.path,flags) ?: error("无法读取APK，请重新上传完整安装包")
                        ...
                        activity.runOnUiThread {
                            try {
                                val uri=Uri.parse("content://"+activity.packageName+".xiaoxingupdates/update.apk")
...
class XiaoxingUpdateProvider:ContentProvider() {
    ...
    override fun openFile(uri:Uri,mode:String):ParcelFileDescriptor {
        require(uri.path=="/update.apk" && mode=="r")
        return ParcelFileDescriptor.open(File(context!!.cacheDir,"xiaoxing-update.apk"),ParcelFileDescriptor.MODE_READ_ONLY)
    }
