...
- 2026-09-13 已部署配套推送后端，TestFlight 真机已确认收到通知；后续原生界面修改仍需重新打包上传。
...
退出登录将解绑请求持久化，可在登录凭据过期后凭随机安装密钥重试。账号切换会更换服务端 binding ID，旧队列不会转发给新账号。按用户要求，通知预览显示该接收人的消息正文，超长内容截断，不读取其他人的任务或全局事件概述。用户可在 iOS 通知设置中控制锁屏预览。离线退出后，已经交给 APNs 的通知无法撤回，旧通知中已显示的正文也无法收回。
